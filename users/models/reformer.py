...
class Reformer(models.Model):
    ...
    reformer_area = models.CharField(max_length=100, blank=True, null=True) # 리포머 활동 지역
...
class ReformerEducation(models.Model):
    ...
    education_uuid = models.UUIDField(primary_key=True, null=False, unique=True, default=uuid.uuid4)
...
class ReformerCertification(models.Model):
    ...
    certification_uuid = models.UUIDField(primary_key=True, null=False, unique=True, default=uuid.uuid4)
...
class ReformerAwards(models.Model):
    ...
    award_uuid = models.UUIDField(primary_key=True, null=False, unique=True, default=uuid.uuid4)
...
class ReformerCareer(models.Model):
    ...
    career_uuid = models.UUIDField(primary_key=True, null=False, unique=True, default=uuid.uuid4)
...
class ReformerFreelancer(models.Model):
    ...
    freelancer_uuid = models.UUIDField(primary_key=True, null=False, unique=True, default=uuid.uuid4)
    project_name = models.CharField(max_length=100) # 프로젝트 이름
    description = models.TextField() # 프리랜서/외주 수행 시 어떤 일을 했는지에 대한 설명을 저장하는 필드
    proof_document = models.FileField(upload_to=get_reformer_certification_upload_path, null=True, blank=True)
