...
class Order(TimeStampedModel):
    ...
    portfolio_status = models.BooleanField(
        default=False
    )  # 해당 주문 포트폴리오 사용 가능 여부
    additional_request = models.TextField(null=True)  # 추가 요청 사항
    payment_status = models.BooleanField(default=False)  # 입금 여부
    material_received_status = models.BooleanField(
        default=False
    )  # 주문자가 발송한 재료를 리포머가 수령했는지에 대한 여부
...
class OrderCategory(TimeStampedModel):
    ...
    order = models.ForeignKey(
        "order.Order", on_delete=models.CASCADE, related_name="order_category"
    )
...
class OrderReformStyle(TimeStampedModel):
    # 리폼 스타일 정보를 저장하는 테이블
    order = models.ForeignKey(
        "order.Order", on_delete=models.CASCADE, related_name="order_style"
    )
...
class OrderReformFit(TimeStampedModel):
    ...
    order = models.ForeignKey(
        "order.Order", on_delete=models.CASCADE, related_name="order_fit"
    )
