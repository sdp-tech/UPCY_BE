...
def get_market_thumbnail_upload_path(instance, filename):
    ...
    return f"users/{email_name}/market/{market_uuid}/thumbnail/{filename}"
...
def get_service_image_upload_path(instance, filename):
    ...
    return f"users/{email_name}/market/{market_uuid}/service/{service_uuid}/image/{filename}"
...
class ServiceMaterial(TimeStampedModel):
    ...
    market_service = models.ForeignKey('market.Service', on_delete=models.CASCADE, related_name='service_material')
...
class ServiceStyle(TimeStampedModel):
    # 해당 서비스가 제공하는 스타일
    market_service = models.ForeignKey('market.Service', on_delete=models.CASCADE, related_name='service_style')
...
class ServiceOption(TimeStampedModel):
    ...
    market_service = models.ForeignKey('market.Service', on_delete=models.CASCADE, related_name='service_option')
...
class ServiceImage(TimeStampedModel):
    # 서비스 소개 이미지 관리 테이블
    market_service = models.ForeignKey('market.Service', on_delete=models.CASCADE, related_name='service_image')
