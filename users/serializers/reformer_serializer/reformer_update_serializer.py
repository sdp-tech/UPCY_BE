...
class ReformerUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if (
            "reformer_link" in attrs
            and attrs["reformer_link"] == self.instance.reformer_link
        ):
            raise serializers.ValidationError("reformer_link must be different")

        if (
            "reformer_area" in attrs
            and attrs["reformer_area"] == self.instance.reformer_area
        ):
    ...
    def update(self, instance, validated_data):
        instance.reformer_link = validated_data.get(
            "reformer_link", instance.reformer_link
        )
        instance.reformer_area = validated_data.get(
            "reformer_area", instance.reformer_area
        )
        instance.save()
        return instance
